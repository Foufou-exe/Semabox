...
class Speedtest:
    ...
    FILE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "TESTMB.zip") # Chemin d'accès au fichier de test à télécharger 
    ...
    async def get_download_speed(url=TEST_FILE_URL) -> str:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response: # Téléchargement du fichier
                ...
                download_speed = len(await response.read()) / elapsed_time # Vitesse de téléchargement
    ...
    async def get_upload_speed(file_path=FILE_PATH,url=UPLOAD_URL) -> str:
        with open(file_path, 'rb') as file: # Ouverture du fichier
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(url, data=files) as response: # Envoi du fichier
                    upload_end_time = time.time() # Fin timer de l'envoi
                    upload_speed = len(await response.read()) / (upload_end_time - upload_start_time) # Vitesse d'envoi
